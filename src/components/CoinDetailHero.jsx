...
const CoinDetailHero = ({ id }) => {
  const { state } = useContext(CoinContext);
  const { coin, BASE_URL } = state;
  ...
  useEffect(() => {
    ...
    axios.get(`${BASE_URL}/assets/${id}`).then((res) => {
      if (isMounted) setLoadedCoin(res.data.data); //after async action
    });

    return () => (isMounted = false);
  }, [BASE_URL, id, loadedCoin]);
              ...
              <p>
                {!valToConvert
                  ? isUsd
                    ...
                    : `$0`
                  : isUsd
                  ? `${valToConvert / loadedCoin.priceUsd} ${loadedCoin.symbol}`
